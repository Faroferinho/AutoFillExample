...
public class Suggestor {
	...
	public Suggestor(JTextField jTextField, Window window) {
		localTxtFld = jTextField;
		localWin = window;
	...
	public Suggestor(JTextField userInput, Window window, float opacity) {
	...
	public Suggestor(JTextField userInput, Window window, Color backgroudColor, Color foregroudColor, Color borderColor) {
		localTxtFld = userInput;
		localWin = window;
	...
	public Suggestor(JTextField userInput, Window window, Color backgroudColor, Color foregroudColor, Color borderColor, float opacity) {
		localTxtFld = userInput;
		localWin = window;
		background = backgroudColor;
		foreground = foregroudColor;
	...
	private void addToPanelMatches(String name) {
		SuggestionLabel lbl = new SuggestionLabel(name, border, foreground, this);
	...
	public void keyBiding() {
		...
		localTxtFld.getActionMap().put("Down Released", new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i = 0; i < localPanel.getComponentCount(); i++) {
					if(localPanel.getComponent(i) instanceof SuggestionLabel) {
						((SuggestionLabel)localPanel.getComponent(i)).setIsFocused(true);
						popUpWindow.toFront();
		...
		localPanel.getActionMap().put("Down Released", new AbstractAction(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				ArrayList<SuggestionLabel> suggestionLabelList = getSuggestionList();
				int quantitySuggestions = suggestionLabelList.size();
				
				if(quantitySuggestions > 1) {
					for(int i = 0; i < quantitySuggestions; i++) {
						SuggestionLabel currSggstnLbl = suggestionLabelList.get(i);
	...
	private ArrayList<SuggestionLabel> getSuggestionList() {
		ArrayList<SuggestionLabel> toReturn = new ArrayList<>();
		
		for(int i = 0; i < localPanel.getComponentCount(); i++) {
			if(localPanel.getComponent(i) instanceof SuggestionLabel) {
				toReturn.add((SuggestionLabel) localPanel.getComponent(i));
			}
		}
